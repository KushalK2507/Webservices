...
public class PatientWSClient {
	...
	private static final String PATIENT_SERVICE_URL = "http://localhost:8080/restwsExceptionForClient/services/patientservice";
	...
	public static void main(String[] args) {
		...
		// method.
		Response updateResponse = putTarget.request().put(Entity.entity(response, MediaType.APPLICATION_XML));
		...
		Patient createResponse = postTarget.request().post(Entity.entity(newPatient, MediaType.APPLICATION_XML),
				Patient.class);
		...
		System.out.println("Patient Created Successfully with Patient ID: " + createResponse.getId()
				+ "and Patient Name" + createResponse.getName());
		...
		WebTarget delTarget = client.target(PATIENT_SERVICE_URL).path("/patients").path("/{id}").resolveTemplate("id",
				123);
